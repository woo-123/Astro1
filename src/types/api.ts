// To parse this data:
//
//   import { Convert, SpiceXResponse } from "./file";
//
//   const spiceXResponse = Convert.toSpiceXResponse(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.

export interface SpiceXResponse {
    docs:          Doc[];
    totalDocs:     number;
    offset:        number;
    limit:         number;
...
export class Convert {
    public static toSpiceXResponse(json: string): SpiceXResponse {
        return cast(JSON.parse(json), r("SpiceXResponse"));
    }

    public static spiceXResponseToJson(value: SpiceXResponse): string {
        return JSON.stringify(uncast(value, r("SpiceXResponse")), null, 2);
    }
}
